...
app.post("/admin/users/:id/ban", (req, res) => {
  if (!requireAdmin(req, res)) return;
  db.query("UPDATE users SET status = 'banned' WHERE id = ?", [req.params.id], (err) => {
...
app.post("/admin/users/:id/unban", (req, res) => {
  ...
  db.query("UPDATE users SET status = 'active' WHERE id = ?", [req.params.id], (err) => {
    if (err) return res.status(500).send("Error unbanning user");
...
app.get("/admin/bundles", (req, res) => {
  if (!requireAdmin(req, res)) return;
  db.query("SELECT * FROM bundles ORDER BY id DESC", (err, results) => {
    if (err) return res.status(500).send("Error loading bundles");
    res.json(results || []);
  });
});
...
app.post("/admin/bundles", (req, res) => {
  ...
  const sql = "INSERT INTO bundles (name, description, price, status) VALUES (?, ?, ?, 'active')";
  db.query(sql, [name, description, price], (err, result) => {
    if (err) return res.status(500).send("Error creating bundle");
    res.json({ id: result.insertId });
  });
});

/* ================= ADDITIONAL DATA ROUTES ================= */
app.get("/api/challenges-data", (req, res) => {
  let sql = "SELECT * FROM CHALLENGE_TABLES";
  db.query(sql, (err, results) => {
    if (err) return res.status(500).send(err);
    res.json(results);
  });
